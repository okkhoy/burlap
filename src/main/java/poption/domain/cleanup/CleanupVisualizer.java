...
public class CleanupVisualizer {
	...
	public static StateRenderLayer getStateRenderLayer(int width, int height){
		...
		oopainter.addObjectClassPainter(CleanupWorld.CLASS_ROOM, new RoomPainter(0, 0, width, height));
		oopainter.addObjectClassPainter(CleanupWorld.CLASS_DOOR, new DoorPainter(0, 0, width, height));
		oopainter.addObjectClassPainter(CleanupWorld.CLASS_BLOCK, new BlockPainter(width, height));
		oopainter.addObjectClassPainter(CleanupWorld.CLASS_AGENT, new AgentPainter(width, height));
	...
	public static class AgentPainter implements ObjectPainter, ImageObserver {
		...
		public void paintObject(Graphics2D g2, OOState s, ObjectInstance ob,
			...
			int x = (Integer) ob.get(CleanupWorld.ATT_X);
			int y = (Integer) ob.get(CleanupWorld.ATT_Y);

			float rx = x*width;
			float ry = cHeight - height - y*height;

			String dir = ob.get(CleanupWorld.ATT_DIR).toString();
	...
	public static class BlockPainter implements ObjectPainter, ImageObserver {
		...
		public void paintObject(Graphics2D g2, OOState s, ObjectInstance ob,
			...
			g2.setColor(colorForName(ob.get(CleanupWorld.ATT_COLOR).toString()));
			...
			String colName = ob.get(CleanupWorld.ATT_COLOR).toString();
			String shapeName = ob.get(CleanupWorld.ATT_SHAPE).toString();
			String key = this.shapeKey(shapeName, colName);
			BufferedImage img = this.shapeAndColToImages.get(key);
			if(img == null){
				Color col = colorForName(ob.get(CleanupWorld.ATT_COLOR).toString()).darker();
	...
	public static class RoomPainter implements ObjectPainter {
		...
		public void paintObject(Graphics2D g2, OOState s, ObjectInstance ob, float cWidth, float cHeight) {
			...
			float domainXScale = CleanupWorld.maxRoomXExtent(cws) + 1f;
			float domainYScale = CleanupWorld.maxRoomYExtent(cws) + 1f;
			...
			int top = (Integer) ob.get(CleanupWorld.ATT_TOP);
			int left = (Integer) ob.get(CleanupWorld.ATT_LEFT);
			int bottom = (Integer) ob.get(CleanupWorld.ATT_BOTTOM);
			int right = (Integer) ob.get(CleanupWorld.ATT_RIGHT);

			Color rcol = colorForName(ob.get(CleanupWorld.ATT_COLOR).toString());
	...
	public static class DoorPainter implements ObjectPainter{
		...
		public void paintObject(Graphics2D g2, OOState s, ObjectInstance ob, float cWidth, float cHeight) {
			...
			String lockedState =  ob.get(CleanupWorld.ATT_LOCKED).toString();
